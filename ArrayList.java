...
public class ArrayList implements List {
    ...
    /*
     * Rotates the elements k times, where k is an integer between 0 and the length of the list
     */

    void rotate(int k) {
        /*Handling the illegal argument case */
        if (k < 0 || k > this.length) {
            throw new IllegalArgumentException("Rotation Value should be positive and less than the length of the list");
        }
        /*Handling the unchanged case */
        else if ( k % this.length == 0) {
            return;
        }
        /*We split the linked list at the appropriate index, and permute the two subarrays */
        else {

            Object[] shifted = new Object[this.length];
            for (int i = 0; i < this.length; i++ ) {
                shifted [ (i + k) % this.length ] = items [ i ];
            }
            items = shifted;
        }
    }
